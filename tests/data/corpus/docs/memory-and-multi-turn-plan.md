...
| 3. 滚动摘要 | 长对话不会越聊越贵 | 很多轮之后仍然保持主线 | 保存 `summary`，只带摘要 + 最近几轮 | 已接入 `/chat` |
...
- 每次真正拼进模型上下文的短期历史，仍然只取最近 6 条。
- 当某个会话里“还没被摘要覆盖的新消息”达到 8 条时，后续阶段再触发一次摘要更新。
- 当前已经完成数据库地基、摘要生成服务，并已接入 `/chat` 的 prompt 组装和保存后更新流程。
...
- [x] 新增 `session_summaries` 表
- [x] 新增 `SessionSummaryRecord`
- [x] 新增 `summary_repository.py`
- [x] 支持查询某个会话的当前摘要
- [x] 支持新增或更新某个会话的一条滚动摘要
- [x] 支持统计某个会话里还没被摘要覆盖的新消息数量
- [x] 支持按旧到新读取某个会话里待总结的消息
- [x] 新增 repository 测试
- [x] 新增 `memory_settings.py`，统一保存 `RECENT_HISTORY_LIMIT` 和 `SUMMARY_TRIGGER_COUNT`
- [x] 新增 `SummaryService`
- [x] 支持在旧消息达到阈值时调用模型生成摘要
- [x] 支持已有摘要时，把旧摘要和新增对话合并成新摘要
- [x] 支持只压缩较旧消息，保留最近 6 条原文历史
- [x] 新增摘要服务测试
- [x] 在 `/chat` 里读取摘要
- [x] 在 `/chat` 保存本轮对话后调用 `SummaryService.update_summary_if_needed()`
- [x] prompt 使用 `summary + 最近 6 条历史 + 当前问题`
- [x] 摘要更新失败时不影响正常聊天
- [x] 新增 `/chat` 摘要接入测试
