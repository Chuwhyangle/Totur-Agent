...
function ChatInput({
  ...
  function handleDrop(event) {
    ...
    if (pdf) attachmentProps?.onUpload?.(pdf)
  }
  ...
  return (
    <div ref={composerRef} className={`composer-wrap${isDragging ? ' is-dragging' : ''}`} onDragOver={(event) => { event.preventDefault(); setIsDragging(true) }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setIsDragging(false) }} onDrop={handleDrop}>
      {isDragging ? <div className="composer-drop-overlay">松开以上传 PDF</div> : null}
      ...
      <div className="composer-options">
        <span className="attachment-control">
          <button className="web-search-toggle attachment-toggle" type="button" aria-label={hasAttachments ? `附件（已选 ${attachments.length} 个）` : '附件'} aria-haspopup="dialog" aria-expanded={attachmentOpen} title="上传附件（当前支持含文本层的 PDF，仅在本次会话有效）" disabled={attachmentProps?.disabled} onClick={() => { if (!hasAttachments) fileInputRef.current?.click(); else setAttachmentOpen((value) => !value) }}><Icon name="paperclip" size={14} /><span>附件</span>{hasAttachments ? <b className="pill-badge">{attachments.length}</b> : null}</button>
          <input ref={fileInputRef} className="attachment-file-input" type="file" accept="application/pdf,.pdf" onChange={async (event) => { const file = event.target.files?.[0]; event.target.value = ''; if (file) await attachmentProps?.onUpload?.(file) }} />
        </span>
