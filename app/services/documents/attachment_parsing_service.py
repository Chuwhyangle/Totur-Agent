...
class AttachmentParsingService:
    ...
    def _parser_identity(self, original_filename: str) -> tuple[str, str]:
        identity_for = getattr(self.parser, "identity_for", None)
        if callable(identity_for):
            return identity_for(original_filename)
    ...
    def parse_attachment(self, document_id: str) -> DocumentRecord:
        ...
        parser_name, parser_version = self._parser_identity(
        ...
        try:
            parsed = self.parser.parse(
                ...
                min_extracted_chars=(
                    ...
                    if parsing.original_filename.lower().endswith(".pdf")
                    else 1
                ),
    ...
    def _mark_failed(
        ...
        *,
        ...
        record = document_repository.get_document(document_id)
        parser_name, parser_version = self._parser_identity(
            record.original_filename if record is not None else ".pdf"
        )
        try:
...
# Compatibility for existing imports while callers migrate to the generic name.
PdfParsingService = AttachmentParsingService
