...
| `requirements.txt` / `.env.example` / `.gitignore` / `README.md` | 修改 | `chromadb` 依赖、独立 Embedding 配置、忽略 `chroma_db/`、文档 | FR3.11 |
...
class EmbeddingConfig:
    api_key: str      # 优先 EMBEDDING_KEY，兼容 EMBEDDING_API_KEY；不复用聊天 key
    base_url: str     # 必须配置 EMBEDDING_BASE_URL；不复用 OPENAI_BASE_URL
    model: str        # 新环境变量 EMBEDDING_MODEL

def load_embedding_config() -> EmbeddingConfig: ...  # 缺 EMBEDDING_KEY/EMBEDDING_BASE_URL/EMBEDDING_MODEL 时 RuntimeError
```
...
其他：`requirements.txt` + `chromadb`；`.env.example` + `EMBEDDING_KEY` / `EMBEDDING_BASE_URL` / `EMBEDDING_MODEL`；`.gitignore` + `chroma_db/`。
