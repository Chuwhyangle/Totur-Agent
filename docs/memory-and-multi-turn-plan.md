...
| 3. 滚动摘要 | 长对话不会越聊越贵 | 很多轮之后仍然保持主线 | 保存 `summary`，只带摘要 + 最近几轮 | 数据库地基已完成 |
...
- 当前阶段只做数据库地基，不改变 `/chat` 的 prompt 组装行为。
...
- [ ] 在 `/chat` 里读取摘要
- [ ] 当未总结消息达到 `SUMMARY_TRIGGER_COUNT` 时生成/更新摘要
- [ ] prompt 使用 `summary + 最近 6 条历史 + 当前问题`
- [ ] 摘要更新失败时不影响正常聊天
